...
function isEqual(a, b) {
  ...
  if (typeof n1 === 'number' && typeof n2 === 'number') return Math.abs(n1 - n2) < 0.01;
...
async function runChecks({
  ...
  // ── PL aggregates ─────────────────────────────────────────
  const { kemasanSum, bruttoSum, nettoSum, kemasanUnit } = hitungKemasanNWGW(sheetPL);
  ...
  if (invCols.headerRow !== null && invCols.cif !== undefined) {
    for (let r = invCols.headerRow + 1; r <= rangeINV.e.r; r++) {
      ...
      if (!qty || isNaN(qty) || !item || String(item).toUpperCase().includes('TOTAL')) break;
  ...
  if (isReturable && parsedExBC.length > 0) {
    for (const doc of parsedExBC) {
      ...
      onResult('Nomor Daftar', draft.nomorText,   invNomorArr.join(', '),   draft.nomorArr.join(',') === invNomorArr.join(','),   { group: 'exbc', isSpecial: true });
      onResult('Tanggal Daftar', draft.tanggalText, invTanggalArr.join(', '), draft.tanggalArr.join(',') === invTanggalArr.join(','), { group: 'exbc', isSpecial: true });
    }
  }
  ...
  onResult('Jenis Transaksi', jenisTransaksi, selectedTrx, jenisTransaksi.toUpperCase() === selectedTrx.toUpperCase());

  const customerDraft = getCustomerDraft(sheetsDATA);
  onResult('Customer', customerDraft, config.check || '', String(customerDraft) === String(config.check || ''));

  const addressDraft = getAddressDraft(sheetsDATA);
  onResult('Address', addressDraft, config.address || '', String(addressDraft) === String(config.address || ''));

  const npwpDraft = getNPWPDraft(sheetsDATA);
  onResult('NPWP', npwpDraft, config.npwp || '', String(npwpDraft) === String(config.npwp || ''), { isSpecial: true });

  const cifDraft = getCellValue(sheetsDATA.HEADER, 'BU2');
  onResult('CIF', `${cifDraft} ${valuta}`, `${cifSum} ${selectedValuta}`, isEqual(cifDraft, cifSum) && valuta === selectedValuta);

  const hargaPenyerahan = getCellValue(sheetsDATA.HEADER, 'BV2');
  onResult('Harga Penyerahan', formatRupiah(hargaPenyerahan), formatRupiah(cifSum * kursParsed), isEqual(hargaPenyerahan, cifSum * kursParsed));

  const dpPajak = getCellValue(sheetsDATA.HEADER, 'CT2');
  const ppnCalc = cifSum * kursParsed * 0.11;
  onResult('PPN 11%', formatRupiah(dpPajak), formatRupiah(ppnCalc), Math.abs((dpPajak || 0) - ppnCalc) < 0.01);
  ...
  onResult('Brutto', getCellValue(sheetsDATA.HEADER, 'CB2'), bruttoSum, isEqual(getCellValue(sheetsDATA.HEADER, 'CB2'), bruttoSum), { unit: 'KG' });
  onResult('Netto',  getCellValue(sheetsDATA.HEADER, 'CC2'), nettoSum,  isEqual(getCellValue(sheetsDATA.HEADER, 'CC2'), nettoSum),  { unit: 'KG' });
  ...
  onResult('Invoice No.',      getDocumentNumber(sheetsDATA.DOKUMEN, '380'), invInvoiceNo,   isEqual(getDocumentNumber(sheetsDATA.DOKUMEN, '380'), invInvoiceNo));
  onResult('Invoice Date',     draftInvoiceDate,     invDateParsed,  isEqual(draftInvoiceDate, invDateParsed));
  onResult('Packinglist No.',   getDocumentNumber(sheetsDATA.DOKUMEN, '217'), plInvoiceNo,    isEqual(getDocumentNumber(sheetsDATA.DOKUMEN, '217'), plInvoiceNo));
  onResult('Packinglist Date',  draftPackinglistDate, plDateParsed,   isEqual(draftPackinglistDate, plDateParsed));

  // Delivery Order
  let invSuratJalan = '';
  if (invCols.suratjalan !== undefined && invCols.headerRow !== null) {
    invSuratJalan = getCellValue(sheetINV, XLSX.utils.encode_cell({ r: invCols.headerRow + 1, c: invCols.suratjalan }));
  }
  const draftDODate = findDocDateByCode(sheetsDATA.DOKUMEN, '640');
  onResult('Delivery Order',      getDocumentNumber(sheetsDATA.DOKUMEN, '640'), invSuratJalan, isEqual(getDocumentNumber(sheetsDATA.DOKUMEN, '640'), invSuratJalan));
  ...
  for (let r = 1; r <= rangeBarang.e.r; r++) {
    ...
    onResult('Code', kodeBarang, invKode, isEqual(kodeBarang, invKode), { group: `barang-${barangCounter}` });
    ...
    onResult('Item Name', draftUraian, invUraian, isEqualStrict(draftUraian, invUraian), { group: `barang-${barangCounter}` });
    ...
    onResult('Quantity', draftQty, invQty, isEqual(draftQty, invQty) && String(effectiveUnit) === String(plUnit), {
      isQty:      true,
      unitForRef:  plUnit,
    ...
    onResult('NW', draftNW, plNW, isEqual(draftNW, plNW), { unit: 'KG', group: `barang-${barangCounter}` });

    const draftGW = getCellValue(sheetsDATA.BARANG, 'U' + (r + 1));
    const plGW    = plCols.gw ? getCellValueRC(sheetPL, rowPL, plCols.gw) : '';
    onResult('GW', draftGW, plGW, isEqual(draftGW, plGW), { unit: 'KG', group: `barang-${barangCounter}` });

    const draftCIF = getCellValue(sheetsDATA.BARANG, 'Z' + (r + 1));
    const invCIF   = invCols.cif ? getCellValueRC(sheetINV, rowINV, invCols.cif) : '';
    onResult('Amount', `${draftCIF} ${valuta}`, `${invCIF} ${selectedValuta}`, isEqual(draftCIF, invCIF) && valuta === selectedValuta, { group: `barang-${barangCounter}` });
