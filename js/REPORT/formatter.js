...
function buildBCGrouped(dataArr, jalurOverrideMap, defaultJalur) {
  ...
  dataArr.forEach(d => {
    const jalur = jalurOverrideMap[d.bc] || jalurOverrideMap[d.aju] || defaultJalur;
...
// ============================================================
function formatBC4x(dataArr, rawBC, bc, arah, jenisBarang, masukTxt, jalurOverrideMap, defaultJalur) {
  ...
  const entitas       = [...new Set(dataArr.map(d => d.entitasBC).filter(Boolean))].join(' | ');

  return [
    `*${rawBC}*`,
    `${labelEntitas} : ${entitas}`,
    ...Object.entries(bcGrouped).map(([k, v]) => `No BC (${k}) : ${v.join(', ')}`),
...
// ============================================================
function formatBC27Keluar(dataArr, jenisBarang, masukTxt, jalurOverrideMap, defaultJalur) {
  const { kemasanMap, barangMap, segelList, tanggalArr } = buildAggregates(dataArr);
  const { bcGrouped } = buildBCGrouped(dataArr, jalurOverrideMap, defaultJalur);

  const pengirim = [...new Set(dataArr.map(d => d.pengirim).filter(Boolean))].join(' | ');
...
function formatBC27Masuk(dataArr, jenisBarang, masukTxt) {
  ...
  return [
    `*BC 2.7 Masuk*`,
    `Supplier : ${pengirim}`,
    ...Object.entries(bcList).map(([j, l]) => `No BC 2.7 (${j}) : ${l.join(', ')}`),
    `No Segel : ${segelList.join(', ')}`,
    `Jumlah Dokumen : ${dataArr.length} Dokumen`,
    `Jenis Barang : ${jenisBarang}`,
    `Jumlah barang : ${formatKeyValue(barangMap)}`,
...
function generateResultText(dataArr) {
  ...
  const key = BC_FORMATTERS[bc]
    ? bc
    : `BC 2.7_${arah}`;

  const formatter = BC_FORMATTERS[key] || BC_FORMATTERS['BC 2.7_Masuk'];
  return formatter(dataArr, rawBC, bc, arah, jenisBarang, masukTxt, jalurOverride, defaultJalur);
}
